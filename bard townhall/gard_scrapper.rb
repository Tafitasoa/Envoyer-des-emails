...
def get_all_the_urls_of_gard_townhalls(x)
    doc = Nokogiri::HTML(open(x))
...
def scrapping
 tableau_final = []
 t = get_all_the_urls_of_gard_townhalls("http://annuaire-des-mairies.com/gard.html")
    t.each do |i|
        tableau_final.push({
...
# On crée un fichier json et on push les resultats de notre scrapping dedans
 
   File.open("gard.json","w") do |f|
        f.write(var.to_json)
   end